function [] = DSR(SRC,RES,srcSuffix)
addpath(genpath('G:\Project\Project\cvpr\Saliency_Matlab\config\DSR\'));

inputpath = SRC;
outputpath = RES;
mkdir(outputpath);
% Files=dir([inputpath  srcSuffix]);
Files = dir(fullfile(inputpath, strcat('*', 'bmp')));
number=length(Files);
%
for num=1:number
...
%     imName = 'test1.bmp';               % Make sure the input image be .bmp file to execute SLICSuperpixelSegmentation.exe.
    imName =  [inputpath Files(num).name];
%     imName =  'F:\Project\Project\cvpr\Saliency_Matlab\config\SOD\2092.bmp';
