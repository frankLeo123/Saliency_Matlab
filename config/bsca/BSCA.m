function [ ] = BSCA( SRC,RES,srcSuffix )
...
addpath('G:\Project\Project\cvpr\Saliency_Matlab\config\bsca\others\');
%%--------------------------set parameters---------------------------%%
...
%%-------------------------Read an input image-----------------------%%                          
imgRoot = SRC;% test image path
saldir = RES;% the output path of the saliency map
mkdir(saldir);
supdir='G:\Project\Project\cvpr\Saliency_Matlab\config\bsca\superpixels\';% the superpixel label file path
mkdir(supdir);
imnames = dir([ imgRoot '*' srcSuffix]);
for ii = 1:length(imnames)
    ...
    [sp_feature_bg] = extractSupfeatBSCA(input_imlab,regions_bg,supNum_bg);                                                                                                    
   ...
   %  K-means Algorithm 
    ...
    % extract LAB features of all superpixels 
    [sp_feature] = extractSupfeatBSCA(input_imlab,regions,supNum);                                                                                                    
